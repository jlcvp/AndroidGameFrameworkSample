...
public class MainScreen extends AndroidScreen {
	...
	public void paint(float deltaTime) {
		...
		g.drawString("FIRE", button_fire.centerX(), button_fire.centerY(), p);
		mFPSCounter.logFrame();
		p.setTextSize(20);
		p.setAntiAlias(true);
		p.setTextAlign(Align.RIGHT);
		p.setColor(Color.YELLOW);
		g.drawString("FPS: "+fps , game.getGraphics().getWidth()-DEFAULT_PADDING, DEFAULT_PADDING, p);
	...
	private void drawProjectile(AndroidGraphics g) {
		g.drawRect(projectile.getPosition().x, projectile.getPosition().y, 3, 12, Color.rgb(32, 255, 32));
